...
window.adsCoffee = {
    ...
    // fetch(`http://localhost:8081/native/${encodeURIComponent(placementId)}`)
      ...
      .then(data => {
        ...
        if (data.network == 'yandex') {
          this.yandex(renderTo, data);
        } else if (data.network == 'coffee') {
          this.coffee(container, template, contentWidth, data);
        }
  ...
  coffee: function(container, template, contentWidth, data) {
    // Создаем стили
